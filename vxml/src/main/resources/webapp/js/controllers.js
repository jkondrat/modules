(function () {
    ...
    var controllers = angular.module('vxml.controllers', []);
    ...
    controllers.controller('SendController', function ($log, $scope, $timeout, $http) {
        ...
        $http.get('../vxml/configs')
        .success(function(response) {
            $scope.config = response;
            $scope.vxml.config = $scope.config.defaultConfigName;
        })
        .error(function(response) {
            $scope.error = $scope.msg('vxml.settings.validate.no_config', response);
        });
    ...
    controllers.controller('SettingsController', function ($scope, $http, $timeout) {
        ...
        $http.get('../vxml/templates')
            .success(function(response){
                $scope.templates = response;
            })
            .error(function(response) {
                 $scope.errors.push($scope.msg('vxml.settings.validate.no_templates', response));
             });
        ...
        $http.get('../vxml/configs')
            ...
            .error(function(response) {
                $scope.errors.push($scope.msg('vxml.settings.validate.no_config', response));
            });
        ...
        */
        ...
        $scope.addConfig = function () {
            ...
            newConfig = {
                ...
                'maxRetries':parseInt($scope.msg('vxml.settings.max_retries.default'), 10),
                'splitHeader':$scope.msg('vxml.settings.split_header.default'),
                'splitFooter':$scope.msg('vxml.settings.split_footer.default'),
                'excludeLastFooter':Boolean($scope.msg('vxml.settings.split_exclude.default'))
                };
        ...
        $scope.betterMsg = function (name, templateName) {
            ...
            if (templateName !== null) {
                key = 'vxml.settings.prop.' + name + '.' + templateName;
                ret = $scope.msg(key);
                if (ret !== '[' + key + ']') {
                    return ret;
                }
            }
            key = 'vxml.settings.prop.' + name;
            ret = $scope.msg(key);
            if (ret === '[' + key + ']') {
                 ret = name;
            }
        ...
        $scope.tooltipOrBlank = function (name, templateName) {
            var key, ret;
            if (templateName !== null) {
                key = "vxml.settings.prop." + name + '.' + templateName + ".tooltip";
                ret = $scope.msg(key);
                if (ret !== "[" + key + "]") {
                    return ret;
                }
            }
            key = "vxml.settings.prop." + name + ".tooltip";
            ret = $scope.msg(key);
            if (ret === "[" + key + "]") {
                ret = "";
            }
        ...
        $scope.submit = function () {
            $http.post('../vxml/configs', $scope.config)
                .success(function (response) {
                    $scope.config = response;
                    $scope.originalConfig = angular.copy($scope.config);
                    setAccordions($scope.config.configs);
                    var index = $scope.messages.push($scope.msg('vxml.settings.saved'));
                    hideMsgLater(index-1);
                })
                .error (function (response) {
                    //todo: better than that!
                    handleWithStackTrace('vxml.error.header', 'vxml.error.body', response);
                });
        };
    });
}());